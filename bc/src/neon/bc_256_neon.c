...
void div_blk( uint32_t *poly, int si_h, int si_l, int polylen )
{
  ...
  for(int i=polylen-1;i>=si_h;i--) {
    uint32x4_t src0 = vld1q_u32( poly+i*8 );
    uint32x4_t src1 = vld1q_u32( poly+i*8+4 );
    uint32x4_t dest0 = vld1q_u32( poly+(i-deg_diff)*8 );
    uint32x4_t dest1 = vld1q_u32( poly+(i-deg_diff)*8+4 );
    dest0 ^= src0;
    dest1 ^= src1;
    vst1q_u32( poly+(i-deg_diff)*8   , dest0 );
    vst1q_u32( poly+(i-deg_diff)*8+4 , dest1 );
  }
}
