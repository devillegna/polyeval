...
static inline
uint64x2_t _vmull_high_p64( uint64x2_t a , uint64x2_t b ) { return vreinterpretq_u64_p128( vmull_high_p64( vreinterpretq_p64_u64(a),vreinterpretq_p64_u64(b) ) ); }
...
uint64x2_t _gf264_mul_neon( uint64x2_t a , uint64x2_t b , uint64x2_t mask_0x1b)
{
  ...
  // reduce
  uint64x2_t xr0 = _vmull_high_p64(x0,mask_0x1b);  // X,XXXXXXXX XXXXXXXX  <-- 3bits in the high 64bits
  uint64x2_t yr0 = _vmull_high_p64(y0,mask_0x1b);  // X,XXXXXXXX XXXXXXXX  <-- 3bits in the high 64bits
  ...
  return yx ^ vreinterpretq_u64_p8(vmulq_p8(vreinterpretq_p8_u64(yrxr),vreinterpretq_p8_u64(mask_0x1b)));
}
...
uint64_t _gf264_mulx1_neon( uint64_t a , uint64_t b )
{
  ...
  uint64x2_t r0 = ab0 ^ ab1r ^ vreinterpretq_u64_p8(vmulq_p8(vreinterpretq_p8_u64(vtrn2q_u64(ab1r,ab1r)),vreinterpretq_p8_u64(mask_0x1b)));  // only low 64 bits is meaningful
